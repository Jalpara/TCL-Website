...
export default function EventsPage() {
  ...
  const events = [
    { id: 1, date: new Date(2026, 6, 25), title: 'Food Drive: The Shared Plate', loc: 'Navi Mumbai', time: '10:00 AM' },
    { id: 2, date: new Date(2026, 6, 28), title: 'Mental Health Workshop', loc: 'Community Center', time: '4:00 PM' },
    { id: 3, date: new Date(2026, 7, 2), title: 'Tree Plantation Drive', loc: 'National Park', time: '7:00 AM' },
    { id: 4, date: new Date(2026, 7, 15), title: 'Literacy Camp Registration', loc: 'City Library', time: '9:00 AM' },
  ];

  const today = new Date(2026, 6, 27); // Standardize today based on current time
                ...
                return (
                  <div key={i} className="bg-white p-3 min-h-[140px] relative border-t border-gray-100 group hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {dayEvents.map(e => (
                         <Link href={`/events/${e.id}`} key={e.id} className="text-xs bg-[#e0f2fe] text-[#0369a1] px-2.5 py-1.5 rounded-lg truncate hover:bg-[#bae6fd] font-bold border border-[#bae6fd] transition-colors block">
                           {e.time} - {e.title}
                         </Link>
                      ))}
                    </div>
